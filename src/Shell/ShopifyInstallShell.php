...
class ShopifyInstallShell extends Shell {
  	...
  	public function main() {
		...
		$this->helper('Shopify.Header')->output();
		...
		if ((($first_run) ? (strtolower($this->in('Install Shopify Plugin?', ['y','n'])) == 'y') : (strtolower($this->in('Update Configuration?', ['y','n'])) == 'y'))) {
			...
			Configure::write('Shopify.api_key',$this->in('API Key:'));
			$this->out();
			Configure::write('Shopify.shared_secret', $this->in('Shared Secret:'));
			$this->out();
			...
			$this->helper('Shopify.Table')->output($scope_array);
			$this->out('');
			...
			Configure::write('Shopify.scope',implode(',', $scope));
			...
			Configure::write('Shopify.is_private_app',$is_private_app);
			
			if($is_private_app == 'true'){
				Configure::write('Shopify.private_app_password',$this->in('Private App Password:'));
			}else{
				Configure::write('Shopify.private_app_password', NULL);
			}
